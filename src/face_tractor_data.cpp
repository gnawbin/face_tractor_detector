...
#include <stdio.h>
#include <string>
void FaceTractorData::rm_incomplete_samples() {
...
void FaceTractorData::draw_points(Mat &im, const int idx, const bool flipped, Scalar color, const vector<int> &pts) {
    if((idx<0)||(idx>=(int)imnames.size()))
        return ;
    ...
    if(pts.size()==0){
        for(int i=0;i<n;i++){
            if(!flipped){
                circle(im,points[idx][i],1,color,2,CV_8U);
            } else{
                Point2f p(im.cols-1-points[idx][symmery[i]].x,points[idx][symmery[i]].y);
                circle(im,p,1,color,2,CV_8U);
            }
        }
...
void FaceTractorData::draw_sym(Mat &im, const int idx, const bool flipped, const vector<int> &pts) {
    if((idx<0)||(idx>=(int)imnames.size()))
         return;
    int n=points[idx].size();
    RNG rn;
    ...
    for(int i=0;i<n;i++){
        if(colors[i]==Scalar::all(0.0)){
            ...
            colors[symmery[i]]=colors[i];
        }
    }
    vector<Point2f> p=this->get_points(idx,flipped);
    if(pts.size()==0){
        for(int i=0;i<n;i++){
            circle(im,p[i],1,colors[i],2,CV_8U);
        }
    }else{
        int m=pts.size();
        for(int j=0;j<m;j++){
...
void FaceTractorData::draw_connect(Mat &im, const int idx, const bool flipped, Scalar color, const vector<int> &con) {
    if((idx<0)||(idx>=(int)imnames.size()))
        return;
    ...
    if(con.size()==0){
        for(int i=0;i<n;i++){
            ...
            if(!flipped){
                ...
                Point2f p(im.cols-1-points[idx][symmery[j]].x,points[idx][symmery[j]].y);
                Point2f q(im.cols-1-points[idx][symmery[k]].x,points[idx][symmery[k]].y);
                line(im,p,q,color,1);
            }
        }
    } else{
        ...
        for(int j=0;j<m;j++){
            ...
            if((i<0)||(i>=n))
                continue;
            int k=connections[i][0];int l=connections[i][1];
            if(!flipped){
                line(im,points[idx][k],points[idx][l],color,1);
            }else{
                Point2f p(im.cols-1-points[idx][symmery[k]].x,points[idx][symmery[k]].y);
                Point2f q(im.cols-1-points[idx][symmery[k]].x,points[idx][symmery[k]].y);
                line(im,p,q,color,1);
            }
        }
...
void FaceTractorData::write(FileStorage &fs) const {
  assert(fs.isOpened());
  fs<<"{";
  ...
  for(size_t i=0;i<connections.size();i++){
      char str[256];
      const char *ss;
      sprintf(str,"connections %s 0",i);
      ss=str;
      fs<<ss<<connections[i][0];
      sprintf(str,"connections %s 1",i);
      fs<<ss<<connections[i][1];
  }
  fs<<"n_symmetry"<<(int)symmery.size();
  for(size_t i=0;i<symmery.size();i++){
      char str[256];
      const char *ss;
...
void FaceTractorData::read(const FileNode &node) {
    ...
    node["n_symmetry"] >> n; symmery.resize(n);
    for(int i = 0; i < n; i++){
        char str[256]; const char* ss;
        sprintf(str,"symmetry %d",i); ss = str; node[ss] >> symmery[i];
    }
